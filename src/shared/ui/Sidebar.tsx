...
export function Sidebar() {
  ...
  const [tab, setTab] = useState<'settings' | 'community'>('settings')
      ...
      <div className="sidebar-panel relative w-full max-w-sm h-full bg-white dark:bg-neutral-900 flex flex-col shadow-[0_0_80px_rgba(0,0,0,0.1)] border-l border-neutral-100 dark:border-neutral-800">
        ...
        <div className="p-6 pt-10 sm:p-8 sm:pt-12">
          ...
          <div className="flex p-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-2xl">
            {(['settings', 'community'] as const).map((tKey) => (
              <button
                key={tKey}
                onClick={() => setTab(tKey)}
                className={`flex-1 py-2.5 rounded-[14px] text-[10px] font-black tracking-[0.15em] transition-all uppercase ${
                  tab === tKey
                    ? 'bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white shadow-sm'
                    : 'text-neutral-400 dark:text-neutral-500'
                }`}
              >
                {tKey === 'settings' ? t('sidebar.settings') : t('sidebar.community')}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto px-6 sm:px-8 py-2 no-scrollbar">
          {tab === 'community' ? (
