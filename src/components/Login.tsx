...
import { ShieldCheck, Chrome, Loader2, ArrowRight, User as UserIcon, Check, AlertCircle, Eye } from 'lucide-react';
import { useGoogleLogin } from '@react-oauth/google';
import { ADMIN_EMAILS } from '../config/constants';
import { upsertProfile, getProfile } from '../services/supabaseService';
...
const Login: FC<LoginProps> = ({ onLogin, lang }) => {
  const [step, setStep] = useState<'initial' | 'profile-setup'>('initial');
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const { email, name, sub: id } = userInfo;
        const role = getRole(email);

        const { data: existingProfile } = await getProfile(id);

        if (existingProfile) {
          onLogin({ ...existingProfile, role } as User);
          setIsConnecting(false);
          return;
        }

        setTempUser({ id, email, name: name || email.split('@')[0], role });
        setCustomName(name || email.split('@')[0]);
    ...
    onError: () => {
      setError(t('loginGoogleFail', lang));
      setIsConnecting(false);
    }
  });
  ...
  const roleBadge = tempUser?.role === 'guest' ? (
        ...
        {step === 'initial' ? (
          <div className="animate-in fade-in zoom-in-95 duration-1000 slide-in-from-bottom-8">
            ...
            <div className="relative group">
              ...
              <button
                onClick={() => !isConnecting && loginWithGoogle()}
