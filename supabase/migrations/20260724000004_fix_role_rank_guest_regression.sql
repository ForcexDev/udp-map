-- 20260724000002 reemplazó el mapeo de rangos por array_position(), pero
-- array_position(ranks, 'guest') = 1, así que "new_rank := array_position(...) - 1"
-- da 0 en lugar de NULL para new_role = 'guest'. Eso saltó el "if new_rank is null
-- then raise exception" y permitió degradar a cualquier usuario a 'guest' desde el
-- panel, algo que la versión original (20260724000001) bloqueaba explícitamente
-- (solo student/moderator/admin son roles asignables por esta RPC).
--
-- No existe tabla de auditoría de cambios de rol, así que no es posible distinguir
-- retroactivamente un 'guest' legítimo (registro nuevo) de uno producido por este bug;
-- no hay backfill de datos que aplicar aquí, solo el cierre del hueco de seguridad.
